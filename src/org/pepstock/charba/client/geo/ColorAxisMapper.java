...
final class ColorAxisMapper extends LegendAxisMapper {
	...
	ColorAxisMapper(Axis axis, NativeObject nativeObject) {
		...
		// sets function to proxy callback in order to invoke the java interface
		this.quantizeCallbackProxy.setCallback(context -> ScriptableUtil.getOptionValueAsNumber(new ScaleContext(getAxis(), context), getQuantizeCallback(), ColorAxis.DEFAULT_QUANTIZE, ScriptableIntegerChecker.POSITIVE_OR_DEFAULT).intValue());
	}
