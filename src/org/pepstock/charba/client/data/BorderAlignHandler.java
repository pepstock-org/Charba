...
final class BorderAlignHandler extends NativeObjectContainer {
	...
	BorderAlignHandler(NativeObject nativeObject, IsDefaultOptions defaultValues) {
		...
		this.borderAlignCallbackProxy.setCallback((contextFunction, context) -> ScriptableUtils.getOptionValueAsString(new DatasetContext(new DataEnvelop<>(context)), getBorderAlignCallback(), defaultValues.getElements().getArc().getBorderAlign()).value());
	}
