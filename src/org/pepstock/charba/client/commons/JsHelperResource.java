...
public final class JsHelperResource extends AbstractInjectableResource {
	...
	private static final String[] CONTENT = {
		...
		"CharbaJsChartHelper.getTooltipActiveElements=function($chart$$){return null!=$chart$$&&null!=$chart$$.tooltip?$chart$$.tooltip.getActiveElements():[]};function CharbaJsZoomHelper(){}CharbaJsZoomHelper.resetZoom=function($chart$$,$mode$$){null!=$chart$$&&\"function\"===typeof $chart$$.resetZoom&&$chart$$.resetZoom.call($chart$$,$mode$$)};CharbaJsZoomHelper.pan=function($chart$$,$amount$$,$mode$$){null!=$chart$$&&\"function\"===typeof $chart$$.pan&&$chart$$.pan.call($chart$$,$amount$$,void 0,$mode$$)};",
		...
		"CharbaJsGeoHelper.features=function($topojson_topojsonObject$$,$featureProperty_parsedFeatures$$){$topojson_topojsonObject$$=JSON.parse($topojson_topojsonObject$$);console.log($topojson_topojsonObject$$);return null!=$topojson_topojsonObject$$&&\"undefined\"!==typeof $topojson_topojsonObject$$.objects&&\"undefined\"!==typeof $topojson_topojsonObject$$.objects[$featureProperty_parsedFeatures$$]&&($featureProperty_parsedFeatures$$=ChartGeo.topojson.feature($topojson_topojsonObject$$,$topojson_topojsonObject$$.objects[$featureProperty_parsedFeatures$$]),",
		"null!=$featureProperty_parsedFeatures$$)?$featureProperty_parsedFeatures$$.features:null};"
	};
