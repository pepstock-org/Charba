...
public final class ArrayTouch extends Array {

	/**
	 * This method creates new array instance with a variable number of <code>Touch</code> arguments.
	 * 
	 * @param items Touch items to create new array
	 * @return new array instance of canvas.
	 */
	private static native ArrayTouch of(Touch... items);
	 ...
	 * 
	 * @param items Touch items to create new array
	 * @return new array instance of canvas or <code>null</code> if argument is <code>null</code> or length to 0
	 */
	@JsOverlay
	public static ArrayTouch fromOrNull(Touch... items) {
	 ...
	 * 
	 * @param items Touch items to create new array
	 * @return new array instance of canvas or an empty array if argument is <code>null</code> or length to 0
	 */
	@JsOverlay
	public static ArrayTouch fromOrEmpty(Touch... items) {
		// checks if consistent
		if (ArrayUtil.isEmpty(items)) {
			// returns an empty array
			return new ArrayTouch();
		}
		// returns array
		return ArrayTouch.of(items);
	}
	 ...
	 * 
	 * @param items list of canvas to load in the new java script array.
	 * @return new array instance of canvas or <code>null</code> if argument is <code>null</code> or empty
	 */
	@JsOverlay
	public static ArrayTouch fromOrNull(List<Touch> items) {
		// checks if list is null
		if (!ArrayListHelper.isConsistent(items)) {
			return null;
		}
		// checks if is already a list with array
		if (items instanceof ArrayTouchList) {
			// casts to array list
			ArrayTouchList list = (ArrayTouchList) items;
			// returns array
			return list.getArray();
		}
		// creates the array
		ArrayTouch result = new ArrayTouch();
		// scans all items of list
		for (Touch value : items) {
			// adds elements
			result.push(value);
		}
		// returns the array
	 ...
	 * 
	 * @param items list of canvas to load in the new java script array.
	 * @return new array instance of canvas or an empty array if argument is <code>null</code> or empty
	 */
	@JsOverlay
	public static ArrayTouch fromOrEmpty(List<Touch> items) {
		// checks if is already a list with array
		if (items instanceof ArrayTouchList) {
			// casts to array list
			ArrayTouchList list = (ArrayTouchList) items;
	 ...
	 */
	native ArrayTouch slice(int start, int end);
	 ...
	 */
	native ArrayTouch splice(int start);
	 ...
	 *         If no elements are removed, an empty array is returned.
	 */
	native ArrayTouch splice(int start, int deleteCounts);
	 ...
	 *         If no elements are removed, an empty array is returned.
	 */
	native ArrayTouch splice(int start, int deleteCounts, Touch item);
	 ...
	 */
	@JsOverlay
	void insertAt(int index, Touch item) {
		splice(index, 0, item);
	}
	 ...
	 */
	native void fill(Touch item, int start, int end);
