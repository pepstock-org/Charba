 ...
 *
 */
public class Layout extends BaseModel<NativeLayout, Options, IsDefaultOptions> {

	private final Padding padding;
	
	public Layout(Options options, IsDefaultOptions defaultOptions) {
		this(new NativeLayout(), options, defaultOptions);
	}

	Layout(NativeLayout delegated, Options options, IsDefaultOptions defaultOptions) {
		super(delegated, options, defaultOptions);
		NativePadding paddingObject = getDelegated().getPadding();
		if (paddingObject != null) {
			padding = new Padding(paddingObject, this, defaultOptions.getPadding());
		} else {
			padding = new Padding(this, defaultOptions.getPadding());
		}
	}
